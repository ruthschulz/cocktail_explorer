...
#define DRINKSIZE		200		//the max size of a drink in ml
#define DELAY			3		//the delay after ordering a drink in seconds
...
#define EDITINGREDIENT	12		//make your own edit ingredient
...
int selectMiddleDrink = -1;	// the button for order the middle drink
int editMiddleDrink = -1;	// the button for edit the middle drink
int cancelMiddleDrink = -1;	// the button for cancel the middle drink
int editIngredientIndex = -1;	// the index for the ingredient edited
...
void initPipeAmounts(void)
{
	...
	std::ifstream b_file("pipeingredients.txt");
	b_file.getline(str, 50, '\n');
	while (strcmp(str, "end") != 0) {
		strcpy_s(pipeIngredients[a].name, str);
		b_file.getline(str, 50, '\n');
		...
		if (pipeIngredients[a].pipeNum <= MAXPIPES) {
			...
			strcpy_s(pipeIngredients[a].name, "");
...
void writePipeAmounts(void) {
	...
	const char *t;
	int decimal, sign;
	std::ofstream b_file("pipeingredients.txt");
...
void initDrinks(void)
{
	...
	while (strcmp(str, "last") != 0) {
		...
		while (strcmp(str, "end") != 0) {
			...
			strcpy_s(drinks[index].textIngredientsAmounts[a], drinks[index].ingredient[a]);
			strcat_s(drinks[index].textIngredientsAmounts[a], " ");
			count = 0;
...
void initMYODrink(void)
{
	...
	for (a = 0; a < MAXINGREDIENTS; a++) {
		...
		strcpy_s(MYODrink.amountString[a], "");
		strcpy_s(MYODrink.ingredient[a], "");
		strcpy_s(MYODrink.textIngredientsAmounts[a], "");
	}
	strcpy_s(MYODrink.name, "Make your own");
	MYODrink.numIngredients = 0;
	for (a = 0; a < 5; a++)
		strcpy_s(MYODrink.textIngredients[a], "");
...
void display(void)
{
	...
	if (drinkOrdered == 1) {
		glPushMatrix();
		glTranslated(x4 + 1.2, -yinit + 0.4, 0.0);
		...
		while (strcmp(lastDrink.ingredient[a], "null") != 0) {
			if (a < 8) {
				...
				displayText(2.2, 0.0 - ((a - 8)*0.2), 0.0, 1.0, 1.0, 1.0, lastDrink.textIngredientsAmounts[a], 0.5);
			}
			else {
				displayText(4.2, 0.0 - ((a - 16)*0.2), 0.0, 1.0, 1.0, 1.0, lastDrink.textIngredientsAmounts[a], 0.5);
			}
			a++;
		}
		glPopMatrix();
...
void drawMYOAmounts(int index)
{
	...
	strcpy_s(MYODrink.ingredient[MYOindex], ingredients[index]);
	initBalls();
...
void drawMYODrink(int index)
{
	...
	if (index == -2) {
		...
		if (MYOcheck == 1) {
			...
			strcpy_s(MYODrink.textIngredientsAmounts[MYOcheckIndex], MYODrink.ingredient[MYOcheckIndex]);
			strcat_s(MYODrink.textIngredientsAmounts[MYOcheckIndex], " ");
			//strcat(MYODrink.textIngredientsAmounts[MYOcheckIndex], (_fcvt(MYODrink.amount[MYOcheckIndex], 0, &decimal, &sign)));
			strcat_s(MYODrink.textIngredientsAmounts[MYOcheckIndex], "ml");
		}
		else {
			MYODrink.amount[MYOindex] = MYOAmounts[index];
			strcpy_s(MYODrink.amountString[MYOindex], MYOTextAmounts[index]);
			strcpy_s(MYODrink.textIngredientsAmounts[MYOindex], MYODrink.ingredient[MYOindex]);
			strcat_s(MYODrink.textIngredientsAmounts[MYOindex], " ");
			strcat_s(MYODrink.textIngredientsAmounts[MYOindex], MYOTextAmounts[index]);
			for (i = 0; i < MAXINGREDIENTS; i++) {
				if (strcmp(MYODrink.ingredient[MYOindex], ingredients[i]) == 0) {
					selected[i] = 1;
				}
...
void removeMYOIngredient(int ball)
{
	...
	while (strcmp(MYODrink.ingredient[a], "null") != 0) {
		strcpy_s(MYODrink.ingredient[a], MYODrink.ingredient[a + 1]);
		MYODrink.amount[a] = MYODrink.amount[a + 1];
		strcpy_s(MYODrink.amountString[a], MYODrink.amountString[a + 1]);
		strcpy_s(MYODrink.textIngredientsAmounts[a], MYODrink.textIngredientsAmounts[a + 1]);
		a++;
	}
	MYOindex--;
	MYODrink.numIngredients = MYOindex;
...
void increaseMYOIngredient(int ingrIndex, double amountIncreased) {
	...
	strcpy_s(MYODrink.textIngredientsAmounts[ingrIndex], MYODrink.ingredient[ingrIndex]);
	strcat_s(MYODrink.textIngredientsAmounts[ingrIndex], " ");
	//strcat(MYODrink.textIngredientsAmounts[ingrIndex], (_fcvt(MYODrink.amount[ingrIndex], 0, &decimal, &sign)));
	strcat_s(MYODrink.textIngredientsAmounts[ingrIndex], "ml");
}
...
void orderDrink(struct drink order) {
	...
	while (strcmp(newChosenDrink.ingredient[a], "null") != 0) {
		...
		tG = 0.0;
		tB = colourSet * 0.2;
		t = newChosenDrink.textIngredientsAmounts[a];
		makeButton(j, x, y, z, s, r, g, b, tS, tR, tG, tB, t, nT, sel, d);
		if (colourSet != 5) {
			colourSet = colourSet + 1;
		}
...
void setArrowDrinkCenter(int arrowIndex) {
	...
	setChosenDrink(arrowDrinks[number]);
	setArrows();
...
void action(int ball)
{
	...
	case DRINKS:
		...
		else if (ball == next) {				// random drink
